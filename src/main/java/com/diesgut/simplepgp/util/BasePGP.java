package com.diesgut.simplepgp.util;
...
public abstract class BasePGP {

	private static final Logger LOGGER = LoggerFactory.getLogger(BasePGP.class);
	...
	protected interface KeyFilter<T> {
		...
		/**
		 *
		 * @param pgpKey the PGP key to check
	 ...
	 *
	 * @param compressionAlgorithm the compression algorithm constant
	 */
	...
	/**
	 * retrieve the appropriate secret key from the secret key ring collection based
	 * on the key filter
	 *
	 * @param secretKeyRingCollection the PGP secret key ring collection
	 * @param keyFilter               the key filter to apply
	 * @return the secret key or null if none matches the filter
	 * @throws PGPException
	 */
	protected PGPSecretKey retrieveSecretKey(PGPSecretKeyRingCollection secretKeyRingCollection,
			KeyFilter<PGPSecretKey> keyFilter) throws PGPException {
		LOGGER.trace("retrieveSecretKey(PGPSecretKeyRingCollection, KeyFilter<PGPSecretKey>)");
		LOGGER.trace("Secret KeyRing Collection: {}, Key Filter: {}",
				secretKeyRingCollection == null ? "not set" : "set", keyFilter == null ? "not set" : "set");
	 ...
	 *
	 * @param secretKey the secret key stream
	 * @param userId    the user id
	 * @return the applicable secret key or null if none is part of the stream for
	 *         the user id
	 * @throws IOException
	 * @throws PGPException
	 */
	 ...
	 *
	 * @param secretKey the secret key stream
	 * @param keyFilter the filter to apply on the stream
	 * @return the secret key or null if none matches the filter acceptance criteria
	 * @throws IOException
	 * @throws PGPException
	 */
	protected PGPSecretKey findSecretKey(InputStream secretKey, KeyFilter<PGPSecretKey> keyFilter)
		...
		try (InputStream armoredSecretKey = new ArmoredInputStream(secretKey)) {
			...
			PGPSecretKeyRingCollection keyRingCollection = new PGPSecretKeyRingCollection(armoredSecretKey,
	 ...
	 * @throws IOException
	 */
	protected PGPPrivateKey findPrivateKey(InputStream secretKey, final String userId, String password)
			throws PGPException, IOException {
		LOGGER.trace("findPrivateKey(InputStream, String, String)");
		LOGGER.trace("Secret Key: {}, User ID: {}, Password: {}", secretKey == null ? "not set" : "set", userId,
	 ...
	 *
	 * @param secretKey the secret key stream
	 * @param keyId     the required key id
	 * @param password  the password to unlock the private key
	 * @return the applicable private key or null if none is found
	 * @throws PGPException
	 * @throws IOException
	 */
	protected PGPPrivateKey findPrivateKey(InputStream secretKey, final long keyId, String password)
			throws PGPException, IOException {
		LOGGER.trace("findPrivateKey(InputStream, long, String)");
		LOGGER.trace("Secret Key: {}, Key ID: {}, Password: {}", secretKey == null ? "not set" : "set", keyId,
				password == null ? "not set" : "********");
		return findPrivateKey(secretKey, password, new KeyFilter<PGPSecretKey>() {
			@Override
			public boolean accept(PGPSecretKey secretKey) {
	 ...
	 *
	 * @param secretKey the secret key stream
	 * @param password  the password to use to unlock the private key
	 * @param keyFilter the filter ot find the appropriate key
	 * @return the appropriate private key or null if none matches the filter
	 * @throws IOException
	 * @throws PGPException
	 */
	protected PGPPrivateKey findPrivateKey(InputStream secretKey, String password, KeyFilter<PGPSecretKey> keyFilter)
			throws IOException, PGPException {
		LOGGER.trace("findPrivateKey(InputStream, String, KeyFilter<PGPSecretKey>)");
		LOGGER.trace("Secret Key: {}, Password: {}, KeyFilter: {}", secretKey == null ? "not set" : "set",
				password == null ? "not set" : "********", keyFilter == null ? "not set" : "set");
	...
	protected PGPPrivateKey findPrivateKey(PGPSecretKey pgpSecretKey, String password) throws PGPException {
		...
		LOGGER.trace("Secret Key: {}, Password: {}", pgpSecretKey == null ? "not set" : "set",
				password == null ? "not set" : "********");
		PGPPrivateKey result = null;
		PBESecretKeyDecryptor pbeSecretKeyDecryptor = new BcPBESecretKeyDecryptorBuilder(
	 ...
	 *
	 * @param publicKey the input key stream
	 * @param keyFilter the filter to apply
	...
	protected PGPPublicKey findPublicKey(InputStream publicKey, KeyFilter<PGPPublicKey> keyFilter) {
		...
		LOGGER.trace("Public Key: {}, Key Filter: {}", publicKey == null ? "not set" : "set",
				keyFilter == null ? "not set" : "set");
	 ...
	 *
	 * @param publicKeyRing the source public key ring
	 * @param keyFilter     the filter to apply
	 ...
	 *
	 * @param publicKey the public key stream
